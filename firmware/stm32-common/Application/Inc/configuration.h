...
/* Compiled-in tunables. Persistent versioned storage is not implemented yet. */
typedef struct {
    float    pid_kp;
    float    pid_ki;
    float    pid_kd;               /* 0 until measurement noise is characterized */
    float    integral_clamp;
    float    feedforward_gain;     /* calibrated PWM fraction per target RPM */
    ...
    float    accel_limit_rpm_per_s; /* target ramp in RPM/s, not a PWM fraction */
